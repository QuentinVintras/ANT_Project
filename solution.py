...
class AppForm(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle('UV2.1 IHM python : PyQt avec matplotlib')
    ...
    def save_plot(self):
        file_choices = "PNG Files (*.png)|*.png"
        path = QtWidgets.QFileDialog.getSaveFileName(self,"Save file","", file_choices)[0]

        if path:
            self.canvas.print_figure(path, dpi=2000) # self.dpi

            self.statusBar().showMessage('Saved to %s' % path, 2000)
    
    def on_about(self):
        msg = """ UE2.1 IHM Python : using PyQt with matplotlib:
        ...
        """
        QtWidgets.QMessageBox.about(self, "About this exemple", msg.strip())
